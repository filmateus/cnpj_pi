...
Os dados foram obtidos no site da Receita Federal e abrangem o período desde a década de 1960 até os dias atuais. Filtramos as informações para utilizar apenas os dados e não buscaremos aprofundar as as mudanças que ocorreram durante o período analisado. 
...
colors = c("#001219","#9b2226","#005f73","#bc6c25","#0a9396","#6d6875","#94d2bd","#ee9b00")
```
...
inicio_atividade|>
  ...
  dplyr::mutate(variable = str_replace(variable, "n.x", "Start"),
                variable = str_replace(variable, "n.y", "Closure"))|>
  dplyr::rename("Activity" = "variable")|>
  ggplot(aes(x = mes_atividade, y = value / 1000, color = Activity)) +
  geom_line(size = 1) +
  theme_minimal(base_size = 10) +
  scale_color_manual(values = c("red", "gold")) +
  scale_x_date(breaks = scales::date_breaks("3 year"),
               labels = scales::date_format("%Y")) +
  labs(title = "History of CNPJ Openings and Closures in Teresina",
       x = "Year",
       y = "Number of companies (thousands)") +
...
ggplot(final_data, aes(x = mes_atividade, y = Saldo)) +
  geom_line(size = 1, color = "blue") +  # Line plot with specified size and color
  ...
  labs(title = "Balance between Openings and Closures",  # Plot title
       x = "Year",  # X-axis label
       y = "Evolution") +  # Y-axis label
...
estabelecimentos_the|>
  ...
  geom_line(size = 1) +
  theme_minimal(base_size = 10) +
  labs(title = "History of CNPJ Openings in Teresina by Region",
       x = "Year",
...
estabelecimentos_the|>
  ...
  geom_line(size = 1) +  # Add line geometry
  theme_minimal(base_size = 10) +  # Apply minimal theme
  labs(title = "History of CNPJ Closures in Teresina by Region",
       x = "Year",
       y = "Number of companies") +
  scale_color_manual(values = c("red", "gold", "green", "purple", "blue")) +  # Set custom colors for regions
  scale_x_continuous(breaks = seq(min(year(estabelecimentos_the$data_inicio_atividade), na.rm = TRUE),
...
ggplot(diferenca_regiao, aes(x = Ano, y = Diferenca_Acumulada, color = Região)) +
  geom_line(size = 1) +
  geom_text(data = valores_finais_regiao, aes(x = max(diferenca_regiao$Ano), y = Valor_Final, label = Valor_Final, color = Região),
            hjust = -0.1, vjust = 1, size = 3.5, show.legend = FALSE) +
  theme_minimal(base_size = 10) +
  labs(title = "Accumulated Difference of Openings and Closures of CNPJ's in Teresina by Region",
       x = "Year",
       y = "Accumulated Difference of Companies") +
  scale_color_manual(values = c("red", "gold", "green", "purple", "blue")) +
  scale_x_continuous(breaks = seq(min(year(estabelecimentos_the$data_inicio_atividade), na.rm = TRUE),
                                  max(year(estabelecimentos_the$data_inicio_atividade), na.rm = TRUE), by = 3)) +
  scale_colour_manual(values = colors)+        
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
...
ggplot() +
  ...
  scale_fill_gradient(
    name = "Quantidade de CNPJ's",      # Legend title
    low = "#caf0f8",                    # Light blue for low values
    high = "#03045e"                    # Dark blue for high values
  ) +
  ...
  labs(
    ...
    subtitle = "Quantidade de Empresas por Diferença Acumulada",  # Subtitle
...
diferenca_bairro|>
  ...
  geom_text(data = valores_finais_bairro, aes(x = max(diferenca_bairro$Ano), y = Valor_Final, label = Valor_Final, color = bairro),
            hjust = -0.1, vjust = 1, size = 3.5, show.legend = FALSE) +
  theme_minimal(base_size = 10) +
  labs(title = "Accumulated Difference of Openings and Closures of CNPJ's in Teresina by Neighborhood (Top 8)",
       x = "Year",
       y = "Accumulated Difference of Companies",
       color = "Neighborhood") +
  scale_x_continuous(breaks = seq(min(year(estabelecimentos_the$data_inicio_atividade), na.rm = TRUE),
...
estabelecimentos_the|>
  ...
  head(20)|>
  dplyr::mutate(n = format_real(n))|>
  flextable::flextable()|>
  flextable::width(j = "nm_cnae", width = 6)|>
  flextable::theme_zebra() # Adjust  
```
...
estabelecimentos_the|>
  ...
  filter(!is.na(nm_cnae), !is.na(Periodo))|>
  dplyr::group_by(nm_cnae)|>
  dplyr::summarise(n =n(), "Médio Tempo Ativo" = round(mean(Periodo), 2), 
                           "Mediana Tempo Ativo" = round(median(Periodo),2), 
                           Máximo = round(max(Periodo),2))|>
...
# Plot the accumulated difference by year
diferenca_nm_cnae|>
  filter(nm_cnae %in% c(valores_finais_nm_cnae$nm_cnae))|>
  ggplot(aes(x = Ano, y = Diferenca_Acumulada, color =str_wrap(nm_cnae,20))) +
  geom_line(size = 1) +
  ...
  theme_minimal(base_size = 10) +
  labs(title = " CNPJ's History in Teresina by CNAE (Top 8)",
       x = "Year",
       y = "Accumulated Difference of Companies",
       color = "nm_cnae") +
  scale_x_continuous(breaks = seq(min(year(estabelecimentos_the$data_inicio_atividade), na.rm = TRUE),
